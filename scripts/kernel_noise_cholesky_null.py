"""
Cholesky-GP null on kernel noise — generative-matched fresh draws.

Uses the exact synthetic simulator covariance: Σ = σ²(I + δ·K) with exp kernel,
Cholesky in cell space, independent fresh draws per gene. This is the null that
best matches how kernel-noise data is generated.

Compare vs prior SpGP parallel (MEM spectral fresh draw) at same n_perms/epochs.
"""
...
from scripts.msr_null_smoke_test import (
    ...
    SEED,
    base_cfg,
    build_kernel_cholesky_surrogates_from_meta,
)
...
# Prior SpGP parallel n=99 run
PRIOR_SPGP = dict(stat_true=141592.99, null_mean=141691.57, null_std=23.83, p=0.010, rank=1)


def run_cholesky_parallel(S: np.ndarray, A: np.ndarray, meta: dict) -> dict:
    print(f"\n  [Cholesky-GP parallel] epochs={EPOCHS}  n_perms={N_PERMS}", flush=True)
    t0 = time.time()

    surrogates = build_kernel_cholesky_surrogates_from_meta(
        S, A.shape[1], N_PERMS, seed=SEED + 700, meta=meta,
    )
    n_slots = N_PERMS + 1
    ...
    _, out, _ = train_parallel_isodepth_model(
        ...
        s_batched=s_batched,
        a_batched=a_batched,
        model_label=f"CholeskyGP_parallel_e{EPOCHS}",
    )
    ...
    return dict(
        method="cholesky_gp", stat_true=stat_true, stat_perm=stat_perm,
        null_mean=null_mean, null_std=null_std, z=z, p=p, rank=rank, elapsed=elapsed,
    )


def plot_result(r: dict, meta: dict) -> None:
    fig, axes = plt.subplots(1, 2, figsize=(11, 4.5))
    kernel = meta.get("kernel", {})
    fig.suptitle(
        f"Cholesky-GP null — kernel noise {DATASET}\n"
        f"ρ={kernel.get('distance')}µm  δ={meta.get('delta')}  σ={meta.get('sigma')}  "
        f"parallel e={EPOCHS}  n_perms={N_PERMS}",
    ...
    ax = axes[0]
    ax.hist(r["stat_perm"], bins=25, color="#2ca02c", alpha=0.72, edgecolor="k", linewidth=0.5)
    ...
    ax.set_title(
        f"Cholesky-GP (generative match)\n"
    ...
    ax2 = axes[1]
    labels = ["SpGP\n(MEM)", "Cholesky-GP\n(cell space)"]
    nulls = [PRIOR_SPGP["null_mean"], r["null_mean"]]
    errs = [PRIOR_SPGP["null_std"], r["null_std"]]
    trues = [PRIOR_SPGP["stat_true"], r["stat_true"]]
    ps = [PRIOR_SPGP["p"], r["p"]]
    xs = np.arange(2)
    ax2.bar(xs, nulls, yerr=errs, capsize=6, color=["#55a868", "#2ca02c"], alpha=0.75, edgecolor="k")
    ...
    plt.tight_layout()
    out = OUTDIR / f"kernel_noise_cholesky_gp_e{EPOCHS}_n{N_PERMS}_{DATASET}.png"
...
def main() -> None:
    t0 = time.time()
    print("=" * 65)
    print(f"CHOLESKY-GP NULL — {DATASET}")
    print(f"n_perms={N_PERMS}  epochs={EPOCHS}  n_reruns={N_RERUNS}  SEED={SEED}")
    print("=" * 65)

    dataset = load_dataset_cache(CACHE_PATH)
    S, A, meta = dataset.S, dataset.A, dataset.meta
    print(f"\nData: {S.shape[0]} cells × {A.shape[1]} genes")
    print(f"  meta: ρ={meta.get('kernel', {}).get('distance')}µm  "
          f"δ={meta.get('delta')}  σ={meta.get('sigma')}  scale_um={meta.get('scale_um')}")

    r = run_cholesky_parallel(S, A, meta)

    print("\n" + "=" * 65)
    print(f"SpGP parallel n=99:     stat_true={PRIOR_SPGP['stat_true']:.2f}  "
          f"null_mean={PRIOR_SPGP['null_mean']:.2f}  p={PRIOR_SPGP['p']:.3f}  rank={PRIOR_SPGP['rank']}/100")
    print(f"Cholesky-GP n={N_PERMS}: stat_true={r['stat_true']:.2f}  "
          f"null_mean={r['null_mean']:.2f}  p={r['p']:.3f}  rank={r['rank']}/{N_PERMS+1}")
    print("=" * 65)
